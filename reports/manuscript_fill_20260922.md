...
| 项目 | 当前状态 |
|---|---|
| 稿件版本 | `main`保留匿名版；本轮基于`14aab55`，完成摘要／引言主线统一和Results重排。当前交付快照为v35，源码与PDF身份由[验证记录](../notes/writing_branch_20260922/draft_validation.submission_sync_20260925.json)固定；作者资料只存本地。 |
| 最新评审判断 | [本轮收口判断](submission_sync_20260925.md)：仅建议P192作为新的科学验证；R1/R2及可选R2b是复现交付，不相互替代。无需自动增加底座、E3旋转、事后子组或E4；本轮未提交新任务。 |
| 最新文字修订 | [修订报告](submission_sync_20260925.md)：摘要先提出训练后的旋转代价与共享补偿；Results按参数化比较→补偿→迁移、竞争性与机制边界组织。107次数值引用及13个标签全部保留，Fresh96／E1／E3边界不变。 |
| 本轮核验 | v35匿名包中23项测试及CPU算子检查通过；1,617字段与121输入完全保留。从同一份匿名源码在HPC3 CPU编译，正文9页、全文38页，无未定义引用或overfull警告。正文1–9页及附录17、34–38页逐页检查，其余页面总览检查；不冒称全文逐字认证。59份编译输入与当前仓库及匿名包逐字节一致。 |
| 活跃来源锁 | [paper_sources.v10.lock.json](../notes/writing_branch_20260922/paper_sources.v10.lock.json)，121份输入、1,617字段。保留全部旧93份输入与1,522字段的原值、格式和来源，新增95字段；v9及此前旧锁不改写。 |
| 已纳入结果 | Protenix方向／长度／G+及两规模完整四格；OpenFold两规模／两PLM四格；AtlasFold两PLM四格；Fresh96；范数干预；E1阴性预测；E2共享补偿；带符号置换；Dev8三节点曲线；E3锚点干预。 |
| 统计身份 | E3是已观察Confirm96-B上、固定三种子／两个预选旋转的Factor干预；目标区间未统一多重校正。E2平均收益和差距缩小仍按原设计保留；A66的后续Ψ不升级成新的主要检验。上述结果不替代Fresh96。 |
| E3材料 | [可读协议、源码与复算入口](../reproducibility/anchor_intervention/README.md)，含11份JSON、17份封存协议／源码／审计文件；与执行锁和收据绑定。9组训练和864次新预测来自既有完成记录，本轮未重跑。 |
| 已完成但独立保留 | 六组共享重建和三底座18次几何／传播数值仍为独立报告；接口反馈分类已进入方法。先前E3独立审阅中的“尚未纳入”已被本轮整合取代。 |
| 其他独立工作 | B组、SGDM未纳入本稿，本入口不推断其队列进度。E3已收口，不追加配置；E4未启动。本轮未核实时队列。 |
| 候选实施计划 | [Protenix新目标与E2重放](protenix_fresh_e2_replay_plan_20260925.md)：P192固定模型4,800预测；E2完整推理1,824预测＋短训练验收。可选R2b四臂完整训练切片和R3九组重训练分别计数，不自动双开。本轮未选目标、未提交新任务。 |
| 前轮图表／评审 | [review3／4落实](review34_response_20260925.md)：摘要E2效应量、Fresh96主表行、均值区间与小提琴分列；[图表修订](figures_tables_revision_20260925.md)：Table 1单行、主表四位均值。旧验证记录保留各自历史时点。 |
| 匿名材料 | 当前为[PDF](../build/submission_v35/paper.pdf)、[匿名包v35](../build/submission_v35/supplement.zip)和[哈希清单](../build/submission_v35/MANIFEST.json)，均为本机构建产物。包内共247文件（246份内容＋MANIFEST），已验证归档内容及来源哈希。文本、可读二进制内容和最终PDF的已知身份字符串检查通过；不等于穷尽匿名性证明。旧包保留历史身份。 |
| 复现范围 | 一个固定OpenFold模型／目标已有隔离环境FASTA→特征→完整CIF→主指标评分；共享C与新锚点hook均有实际算子CPU检查。E3本轮复算起点是保存分数，不是重生成坐标；全矩阵重预测／E2完整重训练复现未建立。 |

E3的预期平均帮助未获支持：T_A = −0.01200 [−0.02665, +0.00187]，现场旋转臂收益 = −0.00264 [−0.00894, +0.00332]。点估计中Q条件的较小差距同时包含Native下降，两个旋转响应相反。它没有关闭整个recycling，也没有C，因而不检验C×anchor依赖，更不通过排除法证明唯一表达机制。

E2仍支持固定设置中的平均有用缓解；E1预测失败、Fresh96主要交互未建立、Dev8曲线未证明收敛等边界均保留。展示层将E2收益写为B_R^C、E3写为B_R^anchor，封存字段名称与结果不变。本轮只整合既有结果，没有新增训练、面板选择或改动执行锁。

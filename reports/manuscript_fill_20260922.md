...
**补充方案设计：** [架构解释实验计划](architecture_followup_plan_20260924.md)已核对原线程封存的1,160次曲线与18组置换安排，并单列6次共享writer重建候选。仅更新MD，未执行新增候选、未修改既有科学锁；本入口仍不报告实时队列。

**执行前评审修订：** [四项意见落实](architecture_followup_review_response_20260924.md)已补齐零输出参照、同区间曲线分解及源码级实现约定，结论限定为有限预算下的共享近似补偿。六组仍为未执行候选，数值锁与论文正文不变。

...
| 项目 | 当前状态 |
|---|---|
| 当前反馈版 | `main`匿名；本轮基于`4836047`落实逐行逻辑审阅及review5-2／review6-2，供尽早审阅。作者资料不提交。 |
| 科学更新 | [联合整合报告](fresh192_retraining_integration_20260925.md)：Fresh192唯一主要Ψ得到支持；E2重训练保留旋转收益，但额外缓解T_C未重新建立。原结果与重训练分开，不合并为新种子。 |
| 稿件呈现 | [本轮回应](logic_review_response_20260925.md)：开头明确固定预算；Fresh96／Fresh192同级、提前；主文补偿表并列绝对分数及两次三项收益和区间，解释旋转收益接近、Native重复收益更大。附录增加已记录条件／未知差异表，复现声明同步。E1／E3边界保留。 |
...
| 直接核验 | [本轮验证](../notes/writing_branch_20260922/draft_validation.logic_review_20260925.json)：204输入、1830字段和生成文件不变；3项数值键测试通过，编译引用完整。上轮完整分数复算与27项包内测试保留在[历史记录](../notes/writing_branch_20260922/draft_validation.fresh192_repeat_20260925.json)，本轮没有重新执行CIF评分或统计分析。 |
| 当前PDF | [反馈版PDF](../build/feedback_v39/paper.pdf)，由65份当前输入编译；无未定义引用、重复标签或overfull。局部检查两张新增表格；按作者指示未核对页数，最终篇幅与全稿验收留到收尾。 |
...
| 其他范围 | E3完成且阴性，不追加旋转；E4未启动。B组／SGDM等独立工作未据此判断进度。本轮没有提交新实验。 |

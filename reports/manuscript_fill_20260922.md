...
| 项目 | 当前状态 |
|---|---|
| 当前反馈版 | `main`匿名；本轮基于`c83389f`补齐训练定义。作者资料不提交。 |
| 科学更新 | [联合整合报告](fresh192_retraining_integration_20260925.md)：Fresh192唯一主要Ψ得到支持；E2重训练保留旋转收益，但额外缓解T_C未重新建立。原结果与重训练分开，不合并为新种子。 |
| 稿件呈现 | [训练定义修订](training_definition_revision_20260925.md)：完整encoder／Factor／G+层序、初始化及构造随机流、普通正交旋转采样规则。前轮引用与科学主线保留。 |
...
| 直接核验 | [本轮验证](../notes/writing_branch_20260922/draft_validation.training_definition_20260925.json)：历史源码、CPU构造和旋转哈希核对；204输入、1830数值不变；3项数值键测试、编译及局部视觉检查通过。 |
| 当前PDF | [反馈版PDF](../build/feedback_v42_training_definition/paper.pdf)，无未定义引用、重复标签、overfull或最终编译警告。局部检查方法及训练定义；最终篇幅与全稿验收留到收尾。 |
...
| 其他范围 | E3完成且阴性，不追加旋转；E4未启动。B组／SGDM等独立工作未据此判断进度。训练定义修订未改变实验；补偿执行分歧另行诊断。 |

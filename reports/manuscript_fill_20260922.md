...
| 项目 | 当前状态 |
|---|---|
| 当前反馈版 | `main`匿名；本轮基于`9c9c17b`落实引用审查。作者资料不提交。 |
| 科学更新 | [联合整合报告](fresh192_retraining_integration_20260925.md)：Fresh192唯一主要Ψ得到支持；E2重训练保留旋转收益，但额外缓解T_C未重新建立。原结果与重训练分开，不合并为新种子。 |
| 稿件呈现 | [引用修订](citation_revision_20260925.md)：21条实际使用的参考文献；更新正式会议版本、补方法与数据库来源，并明确PDB日期缺项及CATH版本身份。前轮科学主线保持。 |
...
| 直接核验 | [本轮验证](../notes/writing_branch_20260922/draft_validation.citations_20260925.json)：21条书目及引用闭合，204输入和1830数值不变；3项数值键测试、编译及局部视觉检查通过。未重新评分CIF或计算统计结果。 |
| 当前PDF | [反馈版PDF](../build/feedback_v41_citations/paper.pdf)，由65份当前输入编译；无未定义引用、重复标签或overfull，保留一条浮动体位置提示。局部检查引用与来源说明；最终篇幅与全稿验收留到收尾。 |

...
| 项目 | 当前状态 |
|---|---|
| 稿件版本 | `main`保留匿名版；在`aa2176e`上正式整合已完成的E3锚点干预。Discussion简述，附录T保留完整设计、绝对分数、平均及逐旋转结果。作者资料只存本地。 |
...
| 匿名材料 | 本地 `build/anonymous_artifact_v34.zip`，247文件；1,617数值一致；v33的23项测试及两类算子检查通过，v34仅更新作者摘要，代码和数据未变。来源脱敏前后哈希均保存；已知身份字符串扫描通过。旧包不覆盖，作者資料未加入。 |

...
**2026-09-24追加结果审阅（未纳入正文）：** [共享重建与OpenFold几何／传播](shared_reconstruction_openfold_review_20260924.md)。原线程已完成六组共享重建及OpenFold两链六次完整重放；本写作轮只读核对原始记录并更新MD。重建属于优于零输出的部分补偿，未达到低误差；传播测的是预测变化，不是质量提高。Protenix／AtlasFold本轮观测按作者消息记为进行中。活跃来源锁仍为v6，532字段不变；不由此推断其他队列进度。
...
**2026-09-24串行研究设计（未启动）：** [前瞻预测与主动干预方案](prospective_orientation_series_20260924.md)。E1检验新旋转排序；E2增加共享正交通道自由度，E3单独改变残差anchor的历史依赖；E4冻结一个诊断量后接受跨底座／新目标检验。首批候选止于E1，后续不是自动启动的任务清单；正文与v6数值均未改变。

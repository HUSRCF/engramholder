...
| 项目 | 当前状态 |
|---|---|
| 当前反馈版 | `main`匿名；本轮基于`9303cd7`收口 Atlas 传播与校准边界。作者资料不提交。 |
| 科学更新 | [联合整合报告](fresh192_retraining_integration_20260925.md)：Fresh192唯一主要Ψ得到支持；E2重训练保留旋转收益，但额外缓解T_C未重新建立。原结果与重训练分开，不合并为新种子。 |
| 稿件呈现 | [Atlas 收口](atlas_boundary_integration_20260926.md)：主文只增一句，附录五行表加既有通道校准阴性结果；讨论保留互补性、下游利用和优化限制候选。此前外积记号、训练定义及补偿重复结论保持。 |
| 来源与数值 | [v12锁](../notes/writing_branch_20260922/paper_sources.v12.lock.json)，266输入、1851字段。旧204输入哈希及1830完整数值对象不变；新增62输入、21字段。旧锁保留。 |
| 直接核验 | [本轮验证](../notes/writing_branch_20260922/draft_validation.atlas_boundary_20260926.json)：20个观测实例、960条分数与18项校准对比复算一致；编译与新增表格局部检查通过。不是重新计算隐藏张量或重新评分CIF。 |
| 当前PDF | [反馈版PDF](../build/feedback_v45_atlas_boundary/paper.pdf)，无未定义引用或 overfull；underfull 提示保留。最终篇幅与全稿验收留到收尾。 |
| 匿名材料 | [v45补充材料](../build/anonymous_atlas_boundary_v45.zip)含411文件，已同步本轮稿源和有界证据，1851数值保持一致。已知身份字符串扫描与算子检查通过；全稿、二进制匿名性及最终篇幅仍待统一验收。 |
| 已完成任务 | P192、R2六条件／384更新、R3九组／13824更新／864预测均完成。相关协议、分数、实现身份与收据已纳入；不再列为待启动。 |
| 已审阅、尚未纳入 | [同起点通道校准A+B](posttraining_calibration_review_20260925/interpretation.md)：33组／3744预测完成，69项原比较及30项已有补充汇总独立复算一致。两项主要优势未建立，OpenFold联合续训保留次要平均收益。仍未纳入正文、v12或匿名包；本轮新增的是独立的 Atlas Native-only 校准。 |
| 注入时序、报告阶段 | [第一阶段](openfold_injection_schedule_phase1_20260926.md)7,008轨迹／14,112评分／0失败，261项复算通过；All−First主要交互差未建立，First自身交互保留。只进入MD，未纳入正文、v12或匿名包。 |

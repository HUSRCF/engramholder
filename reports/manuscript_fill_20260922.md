...
| 项目 | 当前状态 |
|---|---|
| 当前反馈版 | `main`匿名；本轮基于`1889a69`整合Protenix Fresh192与E2九组同种子重训练，供尽早审阅。作者资料不提交。 |
| 科学更新 | [联合整合报告](fresh192_retraining_integration_20260925.md)：Fresh192唯一主要Ψ得到支持；E2重训练保留旋转收益，但额外缓解T_C未重新建立。原结果与重训练分开，不合并为新种子。 |
| 稿件呈现 | 摘要、引言、结果、讨论、结论同时更新；主图／主表并列Fresh192与Fresh96，E2表并列无C、原有C和重训练有C。E1、E3及OpenFold Fresh96边界保留。 |
...
| 直接核验 | [验证记录](../notes/writing_branch_20260922/draft_validation.fresh192_repeat_20260925.json)：Fresh192的4800条分数、24对比及12个次要分层区间；E2重训练864条新分数和原基线的21对比均复算一致。匿名包27项测试通过。没有在本轮重评分CIF。 |
| 当前PDF | [反馈版PDF](../build/feedback_v38/paper.pdf)，43页，主文结束标签在第10页。编译无未定义引用或overfull；尚未作为9页投稿定稿验收。此前v35的9页记录只属于旧快照。 |
| 匿名材料 | [v38补充材料](../build/feedback_v38/supplement.zip)，340文件；65份编译输入与当前仓库／包内源码逐字节一致。最终PDF与二进制匿名检查、全稿排版仍待提交收尾；当前是反馈版。 |

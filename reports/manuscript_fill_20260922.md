...
| 项目 | 当前状态 |
|---|---|
| 当前反馈版 | `main`匿名；本轮基于`94f1344`落实局部复审，供尽早审阅。作者资料不提交。 |
| 科学更新 | [联合整合报告](fresh192_retraining_integration_20260925.md)：Fresh192唯一主要Ψ得到支持；E2重训练保留旋转收益，但额外缓解T_C未重新建立。原结果与重训练分开，不合并为新种子。 |
| 稿件呈现 | [本轮回应](review_followup_94f1344_response_20260925.md)：修正否定范围，主表标出跨头／跨种子的两类混合后端；明确核心四格计数与Holm校正作用域；摘要首次定义两类头。前轮Fresh192、补偿重复、E1／E3主线保持。 |
...
| 直接核验 | [本轮验证](../notes/writing_branch_20260922/draft_validation.review_followup_94f1344_20260925.json)：204输入、1830字段不变；主表生成仅增加三处后端标记。后端逐记录核对、3项数值键测试和编译通过。先前完整统计复算与27项包内测试保留在[历史记录](../notes/writing_branch_20260922/draft_validation.fresh192_repeat_20260925.json)，本轮没有重新评分CIF或统计检验。 |
| 当前PDF | [反馈版PDF](../build/feedback_v40/paper.pdf)，由65份当前输入编译；无未定义引用、重复标签或overfull。PyMuPDF局部检查主表及表注；按作者指示未核对页数，最终篇幅与全稿验收留到收尾。 |

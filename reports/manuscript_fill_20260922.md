...
| 新提案、未启动 | [OpenFold注入时序](openfold_injection_schedule_plan_20260926.md)：固定模型推理与重训练分开；重训练推荐36组（单旋转／三种子），16并行纯训练约7–10小时，含工程／评测约10–16小时、排队另算。跨轮梯度尚待实测，未提交GPU、未修改数值锁。 |

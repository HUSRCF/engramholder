...
| 项目 | 当前状态 |
|---|---|
| 稿件版本 | `main`；最新E1／E2修订基线 `3ef70fc`。此前v7数值整合为 `06b790f`；当前修订见下方新报告。 |
| 活跃来源锁 | [paper_sources.v8.lock.json](../notes/writing_branch_20260922/paper_sources.v8.lock.json)，70份输入、1,199字段；v7的61份输入及1,136字段／映射不变；新增E1的63字段。v6的46份输入及532字段和来源映射不变；旧锁保留。 |
| 已纳入结果 | 原Protenix方向／长度／匹配G+；新增Train96完整四格和Train384 ESM2/ESMC完整四格；OpenFold Train96、Train384两PLM完整四格；AtlasFold两PLM完整四格；Fresh96主要交互未建立；范数干预与既有机制边界；E1共享重建预测未获支持。 |
| 统计身份 | A66每底座主要量为Confirm96-B ESMC Factor−Rotated；其Ψ等为未统一校正的后续比较。Protenix Train96另以Ψ为主要量，仍是旧面板后续研究。不能替代Fresh96。 |
| 已完成但独立保留 | 六组共享重建、三底座18次几何／传播、18组带符号置换和OpenFold三节点Dev8曲线；新收集结果先入MD，未纳入正文。接口反馈分类已进入方法。 |
| 其他独立工作 | B组、SGDM、E2–E4。E2本地18:28:01快照仍无评分收据；置换已有完整结果回传。没有提交新任务或查询实时调度。 |
| 本轮检查 | [E1／E2定位与正式整合](e1_e2_scope_update_20260924.md)；[验证JSON](../notes/writing_branch_20260922/draft_validation.e1_e2_scope.json)。主文9页、全文31页，无未定义引用／overfull；正文修改段及新增附录已视觉检查，非全稿人工验收。 |
| 匿名材料 | 本地 `build/anonymous_artifact_v19.zip`；旧包保留历史身份。数值重建、匿名字符串检查、CPU测试及算子检查通过。 |
...
**前轮图示编辑：** 适配器图明确Factor/G+两路输入、冻结／训练边界、R与独立U0。交互主图同轴展示16个稠密旋转比较；新增统一统计身份表及头间差距恒等式。带符号置换与曲线见[评审回应](review_71370ad_response_20260924.md)，未混入当前数值锁。

**E1正式整合：** [主要预测假说未建立](e1_prediction_review_20260924.md)，八旋转ρ=0.02381、p=0.48839；E2按独立锁继续。初次审阅保留历史身份；本轮正式进入Discussion与独立附录，明确八旋转检验单位、重建与任务目标区别。E2补偿函数类性质见新报告，不预先填写实验结果。

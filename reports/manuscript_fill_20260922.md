...
| 项目 | 当前状态 |
|---|---|
| 当前反馈版 | `main`匿名；本轮基于`242e34a`纳入两项已完成预算／注入时序研究。作者资料不提交。 |
...
| 稿件呈现 | 主文在Fresh192之后加入预算稳健性，范围段加入时序反例；完整表和定义在附录。Atlas保持既有短段，不扩展机制承诺。 |
...
| 直接核验 | [本轮验证](../notes/writing_branch_20260922/draft_validation.budget_schedule_20260926.json)：3552＋9984评分重建267项汇总、1404字段；4800条父模型／Query历史分数逐项相同。未重训或重新评分CIF。 |
| 当前PDF | [反馈版PDF](../build/feedback_v47_budget_schedule/paper.pdf)。最终篇幅与全稿验收留到收尾。 |
| 匿名材料 | [v47补充材料](../build/anonymous_budget_schedule_v47_final.zip)同步本轮稿源及有界证据；全稿、二进制匿名性及最终篇幅仍待统一验收。 |
...
| 注入时序、重训练阶段 | 最终范围为R1／36组、3552预测，全部完成。原72组锁及范围修订保留，R2/R3不等待、不启动；本轮已纳入正文／v14／补充包。 |

...
**最新实验设计修订：** [组织与统计范围](reports/design_revision_20260926.md)。基于`5809c08`落实四项剩余意见：先数据后训练，Fresh96/Fresh192并列；补齐公式符号；区分目标bootstrap与旋转关联检验，Holm仅校正p值；明确tangent比较和评测标签的使用边界。论文源码只改`03_design.tex`，384输入及1985数值对象不变。[当前PDF](build/feedback_v51_design/paper.pdf)、[匿名补充包](build/anonymous_design_v51_final.zip)。
...
**最新论证修订：** [审稿意见落实](reports/narrative_review_revision_20260926.md)。主线调整为新目标检验、预算延长、注入策略与质量的分离；新增主文四格／行内对比／交互总览，补偿作为机制解释的边界。Ψ明确为有符号对比之差。36组收口因算力预算，修订早于记录中的统一评分；不宣称完成原三旋转矩阵。来源锁v14及1985个数值对象不变。[v48历史PDF](build/feedback_v48_narrative/paper.pdf)及[补充包](build/anonymous_narrative_v48_final.zip)保留；当前交付见上方v51。

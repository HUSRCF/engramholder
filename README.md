...
**新实验提案（未启动）：** [OpenFold注入时序与回收传播](reports/openfold_injection_schedule_plan_20260926.md)。建议先固定原24个适配器，在Confirm96-B比较first／last／all，并保存1、2、4轮前缀；0新增训练，预计7,008条四轮轨迹。重训练需解决first-only在原末轮反传下无有效梯度的问题，公平完整矩阵为72组，未默认授权执行。

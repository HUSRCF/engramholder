...
**当前状态唯一入口：** [稿件状态](reports/manuscript_fill_20260922.md)。写作分支为 `main`，活跃来源锁 **v9**，共 **1,522** 个生成数值字段；入口区分已纳入证据、尚未纳入的独立工作及未完成复现交付。下列报告保留各自历史时点，不是远端队列的实时状态。

**本轮旧评审落实：** [review3／review4与E3状态核对](reports/review34_response_20260925.md)。摘要补E2效应量，主表新增Fresh96独立行；小提琴与均值区间分列，E2表明确区间及舍入口径，CPU示例输出运行环境。1,522字段不变，主文9页；匿名包v30通过。

**E3验收审阅：** [现场anchor补偿假说未获支持](reports/e3_anchor_review_20260925.md)。9组训练、864次新预测已完成；本轮从1,824条保存分数独立复算42项对比，全部一致。两个主要区间跨零、逐旋转响应不同；先保留为独立报告，尚未纳入正文／v9，不追加E3或自动启动E4。
...
**2026-09-25候选实施计划：** [Protenix新目标验证与E2完整重放](reports/protenix_fresh_e2_replay_plan_20260925.md)。建议P192固定四格4,800次预测；E2完整预测重放1,824次，并分开短训练验收与可选九组重训练。仅完成规划，尚未选新目标或提交这两条任务；不修改既有结果。E3已完成验收，结果见上方独立审阅，尚未纳入本稿。
...
**串行研究设计与执行衔接：** [前瞻预测→补偿自由度／状态干预→跨底座验证](reports/prospective_orientation_series_20260924.md)。E1／E2现已完成并纳入v9；E3已完成验收且主要假说未获支持，尚未纳入本文，E4未启动。原设计保留历史身份。本写作轮没有提交或修改任务。该系列的Factor预测／补偿对比不替代Ψ的新目标确认。

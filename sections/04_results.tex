...
four-cell means. Right: target-level $\Psi_i$ distributions after averaging
paired seeds and rotations (96 or 48 targets per row). Violins use a fixed
bandwidth factor of 0.35, stop at observed extrema and have equal maximum width;
points and bars show $\Psi$ and its original conditional 95\% bootstrap interval.
The shaded diamond marks Fresh96's new-target primary test, whose interval
includes zero. Other panels are observed; rows are not independent replications
(Table~\ref{tab:study_roles}). A66 follow-ups lack joint correction; backend
...
\begin{table}[t]
\caption{Complete four-cell C$\alpha$ pair-lDDT means at 1536 updates, displayed
to four decimals. F/G+: three seeds; RF/RG+: three seeds $\times$ three rotations.
PLM denotes added adapter features; Query lacks them and Atlas retains native
AtlasLM-3B. Blank recipe cells inherit the entry above within each predictor.
Study identities are in Table~\ref{tab:study_roles}; Protenix/96 has no L48 run.}

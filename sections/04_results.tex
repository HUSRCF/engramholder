\section{Results}
\label{sec:results}
\subsection{Do signed orientation effects differ between heads?}
...
Fresh192 establishes a larger signed contrast for Factor than G+
...
\caption{Head competitiveness and signed orientation contrasts. Left: Protenix
...
has a larger signed Native--Rotated contrast than G+ (Figure~\ref{fig:interaction}). On Confirm96-B, the factor
...
Native--Rotated gap falls from \result{budget_factor_rotation_1536} to
...
Extra optimization therefore reduces Factor's penalty at the tested budgets.
...
G+'s signed contrast also shifts in point estimate. The interaction's paired
change is $K=\result{budget_K}$ [\result{budget_KLo}, \result{budget_KHi}]:
...
\caption{Absolute quality and signed contrasts in the budget and schedule
...
Train/Dev-calibrated learning rate. Table~\ref{tab:budget_schedule_overview}
shows higher Factor mean scores for All,
...
better schedule, a positive Factor direction effect, or a larger absolute
...
re-established. Starting states were verified identical, and short replays
...
\paragraph{Predicting post-training penalties.}
A prespecified teacher-update reconstruction diagnostic did not establish
its predicted ranking of Native--Rotated penalties across eight tested
